...
export class HttpClientService {
  ...
  public client(name:string,password:string){
    const headers = new HttpHeaders({Authorization: 'Basic '+btoa(name+":"+password)})
   return this.httpClient.get("http://localhost:8080/client",{headers,responseType:'text' as 'json'});
  }

  public admin(name:string,password:string){
    const headers = new HttpHeaders({Authorization: 'Basic '+btoa(name+":"+password)})
   return this.httpClient.get("http://localhost:8080/admin",{headers,responseType:'text' as 'json'});
  }
